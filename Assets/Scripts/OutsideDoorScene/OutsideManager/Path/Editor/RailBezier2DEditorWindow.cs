...
public sealed class RailBezier2DEditorWindow : EditorWindow
{
	...
	private void DrawSegments()
	{
		...
		for (int i = 0; i < map.segments.Count; i++)
		{
			...
			Color curveColor = isSelected
				? Color.yellow
				: Color.white;

			float curveWidth = isSelected
				? 4f
				: 2f;

			// Handles.DrawBezier 会直接绘制三次贝塞尔曲线。
			// p0 是起点，p3 是终点，p1 / p2 是两个曲柄控制点。
			Handles.DrawBezier(
				p0,
				p3,
				p1,
			...
			// 额外绘制采样点预览，方便确认 Bake 后路径点大概分布。
			// 这里不依赖 segment.bakedPoints，而是现场计算少量预览点。
			DrawBezierPreviewPoints(
				p0,
				p1,
				p2,
				p3,
	...
	/// 绘制贝塞尔曲线的预览采样点。
	///
	/// 这些点只是 Editor 视图里的辅助显示，
	/// 不会写入 bakedPoints，也不会影响运行时数据。
	/// </summary>
	/// <param name="p0">
	/// 贝塞尔曲线起点。
	/// </param>
	...
	/// 选中时显示更多预览点。
	/// </param>
	private static void DrawBezierPreviewPoints(
		Vector3 p0,
		Vector3 p1,
		Vector3 p2,
		Vector3 p3,
		bool isSelected)
	{
		...
		{
			float t = i / (float)previewCount;

			Vector3 point = EvaluateCubicBezier(
				p0,
				p1,
				p2,
				p3,
