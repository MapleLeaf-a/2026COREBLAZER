<!-- AUTONOMY DIRECTIVE 鈥?DO NOT REMOVE -->
YOU ARE AN AUTONOMOUS CODING AGENT. EXECUTE TASKS TO COMPLETION WITHOUT ASKING FOR PERMISSION.
DO NOT STOP TO ASK "SHOULD I PROCEED?" 鈥?PROCEED. DO NOT WAIT FOR CONFIRMATION ON OBVIOUS NEXT STEPS.
...
Every commit message must follow the Lore protocol 鈥?structured decision records using native git trailers.
...
<body: narrative context 鈥?constraints, approach rationale>
...
5. **`Constraint:` captures external forces.** API limitations, policy requirements, upstream bugs 鈥?things not visible in the code.
...
Confidence: high
Scope-risk: narrow
Directive: Error handling is intentionally broad (all 4xx) 鈥?do not narrow without verifying upstream behavior
...
- `$name` 鈥?invoke a workflow skill
- `/skills` 鈥?browse available skills
...
<agent_catalog>
Key roles:
- `explore` 鈥?fast codebase search and mapping
- `planner` 鈥?work plans and sequencing
- `architect` 鈥?read-only analysis, diagnosis, tradeoffs
- `debugger` 鈥?root-cause analysis
- `executor` 鈥?implementation and refactoring
- `verifier` 鈥?completion evidence and validation
...
- `.omx/state/` 鈥?mode state
- `.omx/notepad.md` 鈥?session notes
- `.omx/project-memory.json` 鈥?cross-session memory
- `.omx/plans/` 鈥?plans
- `.omx/logs/` 鈥?logs
...
<!-- TASK-INTAKE:ROUTING:START -->
## Engineering task intake routing

- When a user gives a new engineering task that implies code changes, debugging, refactoring, migration, integration, or test work, first determine whether an approved task bundle already exists for the current thread or repository.
- If no current task bundle exists, invoke the `task-intake` skill before planning or implementation.
- `task-intake` must clarify only the missing information needed to produce:
  - a task spec
  - a test spec
  - a machine-readable task bundle
- `task-intake` stops after archive + handoff. It does not implement unless the user explicitly asks to skip intake.
- Prefer minimal question batches. Reuse repository context, existing `.omx/plans/` artifacts, and recent conversation state before asking.
- Treat external contracts, compatibility, lifecycle order, destructive behavior, security boundaries, and required validation gates as user-owned until confirmed.
- Treat local implementation structure, private helper design, and low-risk internal organization as agent-owned unless the user says otherwise.
- If the current request already includes both `task-spec-*` and `test-spec-*`, skip `task-intake` and continue into planning or execution mode.
<!-- TASK-INTAKE:ROUTING:END -->

<!-- OMX:RUNTIME:START -->
<session_context>
**Session:** omx-1779174857442-eaqxxv | 2026-05-19T07:14:21.029Z
